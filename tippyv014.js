...
var create_user = function (newuser) {
	...
	fs.writeFile(__dirname + "/db/balances/" + newuser, JSON.stringify(info), function (err, d) {
		if (err) return console.log('Save User error', null);
		if (d) {
			console.log("║".blue + logo + " + SAVE ".green.dim + "│ ".blue + "@" + newuser + " User Account Made!");
		};
		var title = "@Tippy Account Error:";
		var content = "<b>@tippy - Account: ERROR - </b>Steemit Tip Bot<hr>You didn't have a @tippy account or balance so one was created!<hr><sub>@Tippy - Steemit Text-to-Tip Service - Tip Without Ever Leaving The Page!<br>" + devmsg + "</sub>";

			qc.enqueue(replycomment(wif, parentAuthor, parentPermlink, tippy, permlink, title, content, jsonMetadata, safestreamcount));
}); // END create_user
};

//----- Save Block Height to file
function saveblockdataheight(blockheight, type) {
	...
	if (op["author"] != tippy) {
		...
		if (commentstring.toLowerCase().indexOf("@tippy") >= 0) {
									...
									steem.broadcast.transferToVesting(bank, tippy, cleaned, amountafterfee + " " + tipcurrency, function (err, result) {
										...
										if (result) {
											...
											if (fee > 0) {
												...
												qt.enqueue(transfertip(bank, tippy, owner, fee + " " + tipcurrency, tipcurrency, steemfeemessage));
											} else {
			...
			if (commandparsed.toLowerCase().indexOf("vote") >= 0 || commandparsed.toLowerCase().indexOf("-v") >= 0 ) {
				...
				var balancetable = [
					"| <center>@" + parentAuthor + " Got a <b>" + weightpercent + "%</b></center> |",
					"|:----:|",
					"| <center>Vote power is Generated via RNG (Random Number Generator)</center> |",
					"<hr>",
					tippyfooter
				].join("\n");
				var title = "@Tippy Vote Report:";
				//give owner account 100% votes everytime
				if (op["author"] == owner) {
				   weight = 10000;
					 weightpercent = 100;
				};
				...
				console.log("║".blue + logo + " @ ".green.bold + "CALL".white.bold + " │".blue + " VOTE".white.bold + " at " + weightpercent + "% Power Called from @" + parentAuthor + " on Block #" + blockprocesscommnum);
						...
						fs.readFile(__dirname + "/db/balances/" + parentAuthor, function (err, details) {
							...
							if (tipcurrency == "STEEM") {
								...
								if (amount <= steembalance) {
									...
									if (maybmemo != undefined && maybmemo.length >= 2) {
										steemtipmessage = "Tip from @" + parentAuthor + " via @Tippy Text-to-Tip Service 🤖 " + feepercent + "% Fee: " + fee + " " + tipcurrency + " " + maybmemo;
									}; //END maybememo has content
									if (maybmemo.length <= 2) {
									...
									if (fee > 0) {
										 console.log("║".blue + logo + " $ ".green.bold + "FEES".white.bold + " │".blue + " TIP".white.bold + " from @" + parentAuthor + " Earned " + fee + " " + tipcurrency + " From Fees!");
										var steemfeemessage = "@Tippy Fee: " + feepercent + "% 🤖 " + fee + " " + tipcurrency + " 🤖 Paid by @" + parentAuthor;
										qt.enqueue(transfertip(bank, tippy, owner, tippy, fee + " " + tipcurrency, tipcurrency, steemfeemessage, blockprocesscommnum));
									} //END if fee > 0
								} else {
							...
							if (tipcurrency == "SBD") {
								...
								if (amount <= sbdbalance) {
									...
									if (fee > 0) {
										var steemfeemessage = "🤖 @Tippy Fee: " + feepercent + "% - " + fee + " " + tipcurrency + " - Paid by @" + parentAuthor;
										qt.enqueue(transfertip(bank, tippy, owner, fee + " " + tipcurrency, tipcurrency, steemfeemessage, blockprocesscommnum));
									} //END if fee > 0
								} else {
									var title = "@Tippy Tip Error:";
...
// if the block height is undefined don't save anything!
	if (safestreamblock == undefined) {
					console.log("║".blue + logo + " O".red.bold + " DISK".red.bold + " │".blue + " Chain Height Save ERROR! Block Height is 'undefined'");
		process.exit();
	};
...
// save last scanned irreversible block
	var savenewblockdata = {
		last_block: safestreamblock
	};
	console.log("║".blue + logo + " O".green.bold + " DISK".yellow.bold + " │".blue + " Saving Chain Height at Block #" + safestreamblock);
	// save the blockchain height to disk
	fs.writeFile(__dirname + "/db/stats/blockdata", JSON.stringify(savenewblockdata), function (err, win) {
		savedblock = true;
		if (err) {
			console.log("║".blue + logo + " O".red.bold + " DISK".red.bold + " │".blue + " Chain Height Save ERROR!");
		};
		if (win) {console.log("║".blue + logo + " O".green.bold + " DISK".green.bold + " │".blue + " Chain Height Save Succesful!");
	};
	});
...
// on Uncaught Exceotion warn and show error
process.on('uncaughtException', function (err) {
	console.log("║".blue + logo + " ERROR".red.bold + " │ ".blue + "Service Encountered an Uncaught Exeption.. Info Below:");
  console.log(err);
})
